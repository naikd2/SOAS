<%@ page import="com.thinksoas.data.StudentOutcome; com.thinksoas.report.outcome.*" %>
...
<div class="fieldcontain ${hasErrors(bean: SO_ReportInstance, field: 'outcome', 'error')} required">
	...
	<g:select id="outcome" name="outcome.id" from="${StudentOutcome.list()}" optionKey="id" required="" value="${SO_ReportInstance?.outcome?.id}" class="many-to-one"/>
